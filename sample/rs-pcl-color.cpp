...
// add the followings after the line of 38
...
int main() {
    ...
    while(captureLoop == true) {
        ...
        cv::Mat rgb_im(cv::Size(RGB.get_width(),RGB.get_height()), CV_8UC3, (void*)RGB.get_data(), cv::Mat::AUTO_STEP);

        // Map Color texture to each point
        pc.map_to(RGB);

        //Save the PNG (ADD)
        std::string file_name ="Captured_PNG" + to_string(i) + ".png";
        cv::imwrite(file_name,rgb_im);

        //Show the realsense 
        //cv::imshow("realsense",rgb_im);
        //cv::waitKey(0);
        ...
        cout << "Generating PCD Point Cloud File and PNG File ... " << endl;
        pcl::io::savePCDFileBinaryCompressed(cloudFile, *cloud); // Input cloud to be saved to .pcd
...
void Load_PCDFile(void)
{
    ...
    cout << endl;
    cout << "Press [Q] in viewer to continue. (PCD)" << endl;
...
//Show PNG
void Load_PNGFile(Mat rgb_im)
{
  cv::imshow("Captured_PNG",rgb_im);
  cout << "Press [Q] in viewer to continue (PNG). " << endl;
  auto key = waitKey(0);
  if(key!='q' || key!='Q') cv::destroyAllWindows();
}
